...
//подчинен ли регистратору РС
//Гладилов
Перем мРСПодчиненный Экспорт;
...
Функция ВычислитьЗначениеЯчейки(Знач Выражение,Знач ТекущиеДанные,Знач ТекстЯчейки, Знач ТекстыЯчеек,Знач Результат, Знач Реквизит)
	
	//добавлено. Гладилов, 25.07.2014: ставлю в начало, т.к. в исключении не срабатывает, а после "Возврат" ставить бессмысленно
	...
	Исключение
		...
		ОписаниеИсходнойОшибки = ОписаниеОшибки();
		//Изменен Гладилов, 24.07.2014:
		//мСообщитьОбОшибке(ОписаниеОшибки(),,Реквизит+":"+""""+ТекстЯчейки+"""");
		Если Метаданные.ОбщиеМодули.Найти("СтандартныеПодсистемыКлиентСервер")<>Неопределено Тогда
			Попытка
				СообщениеОбОшибке = Вычислить("СтандартныеПодсистемыКлиентСервер.ИсходнаяПричинаОшибки(ИнформацияОбОшибке()"); //идет внутрь структуры Инфо.Причина.Причина...
			Исключение
				//видимо нет ИсходнаяПричинаОшибки()
				мСообщитьОбОшибке(ОписаниеИсходнойОшибки,,"Ошибка вычисления для реквизита: """+Реквизит+""", по тексту ячейки: """+ТекстЯчейки+""". Текст ошибки ниже:");
			КонецПопытки;
		Иначе
			мСообщитьОбОшибке(ОписаниеИсходнойОшибки,,"Ошибка вычисления для реквизита: """+Реквизит+""", по тексту ячейки: """+ТекстЯчейки+""". Текст ошибки ниже:");
		КонецЕсли;
...
Функция ЗаписатьОбъект(Объект, ТекстыЯчеек = Неопределено)
	...
	ОбработкаПрерыванияПользователя();//добавлено. Гладилов, 25.07.2014
	ВыполнитьЗаписьПосле_ПриЗаписи = Ложь;//Гладилов, 20.07.2018. Добавлено: для использования в ПриЗаписи
	
	//начало блока. Гладилов, 01.08.2014:
	Если ПриЗагрузкеИспользоватьРежимОбменаДанными Тогда
		
		Попытка // Планы обмена свойства ОбменДанными не имеют
			Объект.ОбменДанными.Загрузка = Истина;
		Исключение
			Сообщить("Не удалось установить ОбменДанными.Загрузка");
		КонецПопытки;
		
	КонецЕсли;
	//конец блока. Гладилов, 01.08.2014
	...
	Если Не Отказ  Тогда
		Попытка
			//Объект.Записать();
			//начало блока. Гладилов, 01.08.2014:
			Если РежимЗагрузки = 4 и ПриЗагрузкеИспользоватьРежимОбменаДанными Тогда
				Объект.Записать(Истина, Ложь, Ложь, Ложь);
			Иначе
				Объект.Записать();
			КонецЕсли;
			//конец блока. Гладилов, 01.08.2014
		Исключение
			...
			//ОписаниеОшибки = ОписаниеОшибки();
			Если Метаданные.ОбщиеМодули.Найти("СтандартныеПодсистемыКлиентСервер")<>Неопределено Тогда
				ОписаниеОшибки = Вычислить("СтандартныеПодсистемыКлиентСервер.ИсходнаяПричинаОшибки(ИнформацияОбОшибке()"); //идет внутрь структуры Инфо.Причина.Причина...
			Иначе
				мСообщитьОбОшибке(ИнформацияОбОшибке());
			КонецЕсли;
		КонецПопытки;
	КонецЕсли;
	...
	Если Не Отказ и Не ПустаяСтрока(ПриЗаписиОбъекта) Тогда
		...
		Исключение
			Отказ = Истина;
			ОписаниеОшибки = ОписаниеОшибки();
			//заменять на ...ИсходнаяПричинаОшибки(ИнформацияОбОшибке()) нет смысла: подробнее не будет
		КонецПопытки;
		
		Если Не Отказ И ВыполнитьЗаписьПосле_ПриЗаписи Тогда //Гладилов, 20.07.2018. Добавлено:ВыполнитьЗаписьПосле_ПриЗаписи
			Попытка
				//Объект.Записать();
				//начало блока. Гладилов, 01.08.2014:
				Если РежимЗагрузки = 4 и ПриЗагрузкеИспользоватьРежимОбменаДанными Тогда
	...
	Если ТранзакцияАктивна() Тогда //Гладилов, 17.09.2016. Добавлено:
		Если Не Отказ Тогда
...
Функция ОбработатьСобытиеПослеДобавленияСтроки(Объект, ТекущиеДанные, ТекстыЯчеек)
	...
	Отказ = Ложь;//добавлено. Гладилов, 07.08.2014:
	
	Попытка
		
		Выполнить(ПослеДобавленияСтроки);
		Если Отказ Тогда Возврат Ложь; КонецЕсли;//добавлено. Гладилов, 07.08.2014:
...
Функция ПолучитьМетаданныеИсточника() Экспорт
	...
	//Изменен Гладилов, 17.07.2014:добавляю план расчета; 19.07.2018 добавляю документы
	Если РежимЗагрузки = 0 ИЛИ РежимЗагрузки = 3 ИЛИ РежимЗагрузки = 5 Тогда
	...
	ИначеЕсли РежимЗагрузки = 2 Тогда
		Если Не ПустаяСтрока(ИмяВидаРегистра) Тогда
			//Изменен Гладилов, 11.07.2014:добавлена попытка
			попытка
				Возврат Метаданные.РегистрыСведений[ИмяВидаРегистра];
			исключение
				Сообщить(ОписаниеОшибки());
			КонецПопытки;
			//конец блока. Гладилов, 11.07.2014
		КонецЕсли;
	//начало блока. Гладилов, 22.07.2014:
	ИначеЕсли РежимЗагрузки = 4 Тогда
		Если Не ПустаяСтрока(ИмяВидаРегистра) Тогда
			попытка
				Возврат Метаданные.РегистрыРасчета[ИмяВидаРегистра];
			исключение
				Сообщить(ОписаниеОшибки());
			КонецПопытки;
		КонецЕсли;
	//конец блока. Гладилов, 22.07.2014
	//начало блока. Гладилов, 17.02.2023:
	ИначеЕсли РежимЗагрузки = 6 Тогда
		Если Не ПустаяСтрока(ИмяВидаРегистра) Тогда
			попытка
				Возврат Метаданные.РегистрыНакопления[ИмяВидаРегистра];
			исключение
				Сообщить(ОписаниеОшибки());
			КонецПопытки;
		КонецЕсли;
	//конец блока. Гладилов, 17.02.2023
...
Процедура ОбновитьДанныеТабличногоДокумента(ТабличныйДокумент, БезВопросов = Ложь) Экспорт
	...
	Если (РежимЗагрузки = 0 или РежимЗагрузки = 2 или РежимЗагрузки = 4) и ТабличныйДокумент.ВысотаТаблицы > 1 И Не БезВопросов Тогда
	...
	//Изменен Гладилов, 18.07.2014:добавляю план расчета; 19.07.2018 добавляю документы
	...
	Если РежимЗагрузки <>1 Или МетаданныеИсточника = Неопределено Тогда
		Возврат;
	КонецЕсли;
	...
	Для каждого Строка Из Источник Цикл
		...
		Для каждого ЗагружаемыйРеквизит Из ТаблицаЗагружаемыхРеквизитов Цикл
			...
			Если ЗагружаемыйРеквизит.Пометка Тогда
				...
				// 26.11.2018 -  - Задача № - [Гладилов] Изменено
				Попытка
					Область = ТабличныйДокумент.Область("R" + Формат(НомерСтроки, "ЧГ=") + "C"+НомерКолонки);
				Исключение
					Сообщить(ОписаниеОшибки());
					Сообщить("Непонятный глюк только при открытии: Ошибка при вызове метода контекста (Область). После открытия возникает только если не указан номер колонки в настройках");
					Продолжить;
				КонецПопытки;
...
Функция ЗагрузитьДанные(ТабличныйДокумент, Индикатор) Экспорт
	...
	//начало блока. Гладилов
	ИначеЕсли РежимЗагрузки = 3 Тогда
		
		Источник = СсылкаИсточника;
		ТекстВопросаИсточника = " элементов в план видов расчета: """ + МетаданныеИсточника.Представление() + """";
		
	ИначеЕсли РежимЗагрузки = 4 Тогда
		
		ТекстВопросаИсточника = " записей в регистр расчета: """ + МетаданныеИсточника.Представление() + """";
		
	ИначеЕсли РежимЗагрузки = 5 Тогда
		
		Источник = СсылкаИсточника;
		ТекстВопросаИсточника = " элементов в документ: """ + МетаданныеИсточника.Представление() + """";
	ИначеЕсли РежимЗагрузки = 6 Тогда
		
		ТекстВопросаИсточника = " записей в регистр накопления: """ + МетаданныеИсточника.Представление() + """";
	//конец блока. Гладилов
	...
	Если Вопрос("Загрузить "+КоличествоЭлементов  + ТекстВопросаИсточника, РежимДиалогаВопрос.ДаНет) = КодВозвратаДиалога.Да Тогда
		...
		ИначеЕсли РежимЗагрузки = 1 Тогда
			...
			//Источник.Очистить();
			//Изменен Гладилов, 26.11.2018:
			Если НеОчищатьТЧПередЗагрузкой Тогда
				//ничего
			Иначе
				Источник.Очистить();
			КонецЕсли;
		...
		//ИначеЕсли РежимЗагрузки = 2 Тогда
		//Изменен Гладилов, 22.07.2014:
		ИначеЕсли РежимЗагрузки = 2 Или РежимЗагрузки = 4 Или РежимЗагрузки = 6 Тогда
			...
			//Гладилов; устанавливаем НаборЗаписей один раз
			Если мРСПодчиненный = Истина Или РежимЗагрузки = 4 Или РежимЗагрузки = 6 Тогда
				
				Если мРСПодчиненный = Истина Тогда
					ОбъектОтбор = РегистрыСведений[МетаданныеИсточника.Имя].СоздатьНаборЗаписей();
				ИначеЕсли РежимЗагрузки = 4 Тогда
					ОбъектОтбор = РегистрыРасчета[МетаданныеИсточника.Имя].СоздатьНаборЗаписей();	
				ИначеЕсли РежимЗагрузки = 6 Тогда
					ОбъектОтбор = РегистрыНакопления[МетаданныеИсточника.Имя].СоздатьНаборЗаписей();
				КонецЕсли;
				Док = ОбъектОтбор.Отбор.Регистратор ;
				
				Если Не ЗамещатьСуществующиеЗаписи Тогда
					Док.Установить(КонтрольЗаполненияСтроки(ТабличныйДокумент, 1).Регистратор);
					ОбъектОтбор.Прочитать();
					ОбъектНайден = ОбъектОтбор.Выбран();
				Иначе
					ОбъектНайден = Ложь;
				КонецЕсли;
				
				ОбъектОтбор.Записывать = Истина;
			КонецЕсли;//мРСПодчиненный
			//кблока

		//Гладилов, 19.07.2018, начало блока:
		ИначеЕсли РежимЗагрузки = 5 Тогда
		...
		Загружено = 0;
		//начало блока. Гладилов, 07.08.2014:
		НомерТекСтрока = 0;
		//конец блока. Гладилов, 07.08.2014
		Для К = ПерваяСтрокаДанныхТабличногоДокумента По ТабличныйДокумент.ВысотаТаблицы Цикл
			...
			Если РежимЗагрузки =0 или РежимЗагрузки =5 Тогда //Гладилов, 19.07.2018. Добавлено: или 5
				...
				Если Не ОбъектНайден Тогда
					Сообщить("Строка " + НомерТекущейСтроки + ": объект не найден.",СтатусСообщения.Информация);//Гладилов, 19.07.2018. Добавлено
					Если НеСоздаватьНовыхЭлементов Тогда
						Индикатор.Значение = Индикатор.Значение + 1;
						Продолжить;
					Иначе
						//Объект = Справочники[МетаданныеИсточника.Имя].СоздатьЭлемент();
						//Гладилов, 19.07.2018. изменено:
						Если РежимЗагрузки = 0 Тогда
							Объект = Справочники[МетаданныеИсточника.Имя].СоздатьЭлемент();
						Иначе
							Объект = Документы[МетаданныеИсточника.Имя].СоздатьДокумент();
						КонецЕсли;
					КонецЕсли;
				КонецЕсли;
			...
			ИначеЕсли РежимЗагрузки = 2 Или РежимЗагрузки = 4 Или РежимЗагрузки = 6 Тогда
				...
				Если мРСПодчиненный = Истина Или РежимЗагрузки = 4 Или РежимЗагрузки = 6 Тогда //мРСПодчиненный
					Объект = ОбъектОтбор.Добавить();
				Иначе
				...
				КонецЕсли;//НЕ мРСПодчиненный
				
			//начало блока. Гладилов, 21.07.2014:
			ИначеЕсли РежимЗагрузки = 3 Тогда
				...
				Если Не ОбъектНайден Тогда
					Если НеСоздаватьНовыхЭлементов Тогда
						...
						Продолжить;
					Иначе
						Объект = ПланыВидовРасчета[МетаданныеИсточника.Имя].СоздатьВидРасчета();
					КонецЕсли;
				КонецЕсли;
			...
			Для каждого КлючИЗначение Из ТекущаяСтрока Цикл
				...
				Если Не ОбъектНайден или Колонки[КлючИЗначение.Ключ].Пометка  Тогда
					//Гладилов, 19.07.2018. Добавлено:
					//Для документа в таблице реквизитов поставили флаг на реквизите "Ссылка" - чтобы в свойствах реквизита найти ссылку по другим реквизитам,
					//и теперь объект нашли по ссылке.
					//Сделали так потому что например если ставить Дата+Номер то стандартный запрос ищет "где дата=дата", а у нас не точная дата (дата+время) в xls.
					Если ОбъектНайден И КлючИЗначение.Ключ = "Ссылка" Тогда
						Продолжить;
					КонецЕсли;
					//Гладилов, 19.07.2018, конец блока.
					Попытка
						Объект[КлючИЗначение.Ключ] = КлючИЗначение.Значение;
					Исключение
			...
			ИначеЕсли РежимЗагрузки = 2 Или РежимЗагрузки = 4 Или РежимЗагрузки = 6 Тогда
				...
				Если мРСПодчиненный = Истина Или РежимЗагрузки = 4 Или РежимЗагрузки = 6 Тогда //мРСПодчиненный
					...
					//выполним пользовательские проверки перед тем как добавлять строку в НаборЗаписей
					//Гладилов
					Если Не ОбработатьСобытиеПослеДобавленияСтроки(ОбъектОтбор, Объект, ТекстыЯчеек) Тогда
						Отказ = Истина;
					КонецЕсли;
					
					Если Не Отказ Тогда
						Сообщить("Добавлена в НаборЗаписей строка: " + (Загружено + 1));
						Загружено = Загружено + 1;
					Иначе
						Сообщить("При добавлении в НаборЗаписей строки " + (НомерТекСтрока + 1) + " получен отказ в событии ПослеДобавленияСтроки. ");
						ОбъектОтбор.удалить(объект);
					КонецЕсли;
					НомерТекСтрока = НомерТекСтрока +1;
					
				Иначе // НЕ мРСПодчиненный
					//типовое
					Если Не Отказ и ЗаписатьОбъект(Объект, ТекстыЯчеек) Тогда
			...
			ИначеЕсли РежимЗагрузки = 3 Тогда
				Если Не Отказ и ЗаписатьОбъект(Объект, ТекстыЯчеек) Тогда
					Сообщить(?(ОбъектНайден,"Изменен","Загружен") + " элемент плана видов расчета: " + Объект.ссылка, СтатусСообщения.Информация);
					Загружено = Загружено + 1;
				Иначе
					Сообщить("Объект не " + ?(ОбъектНайден,"изменен","загружен") + ". Элемент плана видов расчета: " + Объект + ".", СтатусСообщения.Важное);
				КонецЕсли;
				
			//Гладилов, 19.07.2018, начало блока:
			ИначеЕсли РежимЗагрузки = 5 Тогда
				Если Не Отказ и ЗаписатьОбъект(Объект, ТекстыЯчеек) Тогда
					Сообщить(?(ОбъектНайден,"Изменен","Загружен") + " документ: " + Объект.ссылка, СтатусСообщения.Информация);
					Загружено = Загружено + 1;
				Иначе
					Сообщить("Объект не " + ?(ОбъектНайден,"изменен","загружен") + ". Документ: " + Объект + ".", СтатусСообщения.Важное);
				КонецЕсли;
			//Гладилов, 19.07.2018, конец блока.
			КонецЕсли;
		...
		Если РежимЗагрузки = 1  Тогда
			...
			Иначе
				...
				Возврат Ложь;
			КонецЕсли;
		ИначеЕсли РежимЗагрузки = 0 ИЛИ РежимЗагрузки = 3 или РежимЗагрузки = 5 Тогда //Гладилов, 19.07.2018. Изменено: добавлены 3 и 5 
			Сообщить("Выполнена загрузка"+ 	ТекстВопросаИсточника, СтатусСообщения.Информация);
			Сообщить("" +Загружено +" из "+ КоличествоЭлементов + " элементов.", СтатусСообщения.Информация);
			Возврат Истина;
		ИначеЕсли РежимЗагрузки = 2 Тогда
			
			Если НЕ мРСПодчиненный Тогда
				
				Сообщить("Выполнена загрузка"+ 	ТекстВопросаИсточника, СтатусСообщения.Информация);
				Сообщить("" +Загружено +" из "+ КоличествоЭлементов + " записей.", СтатусСообщения.Информация);
				Возврат Истина;
				
			Иначе //мРСПодчиненный
				
				Если ЗаписыватьОбъект и ЗаписатьОбъект(ОбъектОтбор, ТекстыЯчеек) Тогда //ТекстыЯчеек передаем т.к. они могут использоваться в ПередЗаписью
					Сообщить("Выполнена загрузка"+ 	ТекстВопросаИсточника, СтатусСообщения.Информация);
					Сообщить("" +Загружено +" из "+ КоличествоЭлементов + " записей.", СтатусСообщения.Информация);
					Возврат Истина;
				Иначе
		...
		//начало блока. Гладилов, 07.08.2014:
		ИначеЕсли РежимЗагрузки = 4 Или РежимЗагрузки = 6 Тогда
	...
	Для каждого КлючИЗначение Из Колонки Цикл
		...
		Если Колонка.Пометка Тогда
			...
			ИначеЕсли Колонка.РежимЗагрузки = "Вычислять" Тогда
				...
				//Изменен Гладилов, 24.07.2014: уточняю в каком реквизите ошибка
				Вычисление  = ВычислитьЗначениеЯчейки(Колонка.Выражение,ТекущаяСтрока,"",ТекстыЯчеек,Колонка.ЗначениеПоУмолчанию, Колонка.ПредставлениеРеквизита);
...
Функция ОбработатьОбласть(Область, Колонка, ТекущиеДанные, ТекстыЯчеек)
	...
	Если Колонка.РежимЗагрузки = "Вычислять" Тогда
		...
		//Изменен Гладилов, 24.07.2014: уточняю в каком реквизите ошибка
		Вычисление = ВычислитьЗначениеЯчейки(Колонка.Выражение,ТекущиеДанные,Представление, ТекстыЯчеек, Колонка.ЗначениеПоУмолчанию, Колонка.ПредставлениеРеквизита);
	...
	Иначе
		...
		Если НайденныеЗначения.Количество() = 0 Тогда
			...
			Примечание = "Не найден "+""""+Представление+""""+?(Примечание = "","", Символы.ПС+Примечание);
			Результат = Неопределено;
...
Функция мПолучитьНастройки() Экспорт
	...
	//добавлено. Гладилов
	ИначеЕсли РежимЗагрузки = 3 Тогда
		ОбластьШапки.Параметры.РежимЗагрузки = "в план видов расчета";
	ИначеЕсли РежимЗагрузки = 4 Тогда
		ОбластьШапки.Параметры.РежимЗагрузки = "в регистр расчета";
	ИначеЕсли РежимЗагрузки = 5 Тогда
		ОбластьШапки.Параметры.РежимЗагрузки = "в документ";
	ИначеЕсли РежимЗагрузки = 6 Тогда
		ОбластьШапки.Параметры.РежимЗагрузки = "в регистр накопления";
	//конец. Гладилов
	КонецЕсли;
	...
	//Изменен Гладилов, 07.08.2014:
	Если РежимЗагрузки = 1 или РежимЗагрузки = 4 Тогда
			...
			Для каждого Владелец Из Метаданные.НайтиПоТипу(ТипКолонки).Владельцы Цикл
				...
				Если ТипыВладельцев[ТипВладельца] = Неопределено Тогда
					...
					Для каждого КолонкаСвязиПоВладельцу Из ТаблицаЗагружаемыхРеквизитов Цикл
						Если КолонкаСвязиПоВладельцу.ОписаниеТипов.типы().количество()>0 и //Гладилов добавлено
							КолонкаСвязиПоВладельцу.ОписаниеТипов.Типы()[0] = ТипВладельца Тогда
...
Функция ПолучитьСписокСвязейПоТипу(ЗагружаемыйРеквизит) Экспорт
	...
	Для каждого ПланСчетов Из Метаданные.ПланыСчетов Цикл
		...
		Если ПланСчетов.ВидыСубконто <> неопределено Тогда
			Если ПланСчетов.ВидыСубконто.Тип  <> неопределено Тогда
...
Процедура ЗаполнитьНастройкиКолонок(Настройки) Экспорт
	...
	Если ТипЗнч(Настройки) = Тип("ТабличныйДокумент") Тогда
		...
		Попытка
			...
			//добавлено. Гладилов
			ИначеЕсли ТекстВосстановленногоРежимаЗагрузки = "в план видов расчета" Тогда
				ВосстановленныйРежимЗагрузки = 3;
			ИначеЕсли ТекстВосстановленногоРежимаЗагрузки = "в регистр расчета" Тогда
				ВосстановленныйРежимЗагрузки = 4;
			ИначеЕсли ТекстВосстановленногоРежимаЗагрузки = "в документ" Тогда
				ВосстановленныйРежимЗагрузки = 5;	
			ИначеЕсли ТекстВосстановленногоРежимаЗагрузки = "в регистр накопления" Тогда
				ВосстановленныйРежимЗагрузки = 6;
			//конец блока. Гладилов
			...
			//Изменен Гладилов, 21.07.2014:
			Если ВосстановленныйРежимЗагрузки = 0 или ВосстановленныйРежимЗагрузки = 3 или ВосстановленныйРежимЗагрузки = 5 Тогда
			...
			Пока Не ПустаяСтрока(ИмяРеквизита) Цикл
				...
				//добавлено. Гладилов, 21.07.2014:
				НомерКолонки = Настройки.Область(ТекущаяСтрокаОбласти + "C11").Текст;
				СтруктураУмолчанияРеквизита.Вставить("НомерКолонки",мПривестиКЧислу(НомерКолонки));
				//конец блока. Гладилов, 21.07.2014
		...
		//добавлено. Гладилов, 08.10.2014:
		Если РежимЗагрузки = 4 Тогда
			ИмяВидаРегистра = МетаданныеОбъекта.Имя;
		МетаданныеИсточника = ПолучитьМетаданныеИсточника();
		КонецЕсли;
		//конец блока. Гладилов, 08.10.2014
	...
	//заполнение настроек загрузки значениями по-умолчанию
	Если      РежимЗагрузки = 0 Тогда
	...
	ИначеЕсли РежимЗагрузки = 1 Тогда
		...
		Если Не ОсновнаяФормаОбъекта = Неопределено и СтруктураУмолчаний = Неопределено Тогда
			...
			Если Не ПустаяСтрока(ИмяОсновнойФормы) Тогда
				...
				//		
				//КонецЦикла;
				//прим Гладилов: вроде я не менял, было закомментировано
			КонецЕсли;
	...
	//добавлено. Гладилов
	ИначеЕсли РежимЗагрузки = 3 Тогда
		ЗаполнитьНастройкиКолонокПланаВидовРасчета();
	ИначеЕсли РежимЗагрузки = 4 Тогда
		ЗаполнитьНастройкиКолонокРегистраРасчета();
	ИначеЕсли РежимЗагрузки = 6 Тогда
		ЗаполнитьНастройкиКолонокРегистраНакопления();
	//конец блока. Гладилов
	ИначеЕсли РежимЗагрузки = 5 Тогда
		ЗаполнитьНастройкиКолонокДокумента();
	КонецЕсли;
	
	//заполнение настроек загрузки значениями из файла
		...
		Для каждого КлючИЗначение Из СтруктураУмолчаний Цикл
			...
			Если Не ЗагружаемыйРеквизит = Неопределено Тогда
				...
				Если Индекс >= НомерКолонкиОформления Тогда
					...
					ТаблицаЗагружаемыхРеквизитов.Сдвинуть(ЗагружаемыйРеквизит, НомерКолонкиОформления - Индекс);
					Если Колонка.Пометка Тогда
						//Изменен Гладилов, 21.07.2014: отключаю автонумерацию
						//ЗагружаемыйРеквизит.НомерКолонки = НомерКолонки;
						НомерКолонки = НомерКолонки + 1;
					КонецЕсли;
					НомерКолонкиОформления = НомерКолонкиОформления + 1;
...
Процедура ЗаполнитьНастройкиКолонокРегистраСведений()
	...
	//09.12.2014Гладилов
	Если МетаданныеИсточника.РежимЗаписи = Метаданные.СвойстваОбъектов.РежимЗаписиРегистра.ПодчинениеРегистратору Тогда
...
//Гладилов, 17.02.2023. Добавлено:
// Заполняет настройки колонок по умолчанию для регистра накопления
Процедура ЗаполнитьНастройкиКолонокРегистраНакопления()
	...
	//Если МетаданныеИсточника.РежимЗаписи = Метаданные.СвойстваОбъектов.РежимЗаписиРегистра.ПодчинениеРегистратору Тогда
	//Гладилов, 02.02.2015. Изменено:это условие имеет смысл только для РС
		Для каждого Реквизит Из МетаданныеИсточника.СтандартныеРеквизиты   Цикл
			ЗагружаемыйРеквизит                        = ТаблицаЗагружаемыхРеквизитов.Добавить();
			ЗагружаемыйРеквизит.ИмяРеквизита           = Реквизит.Имя;
			ЗагружаемыйРеквизит.ПредставлениеРеквизита = Реквизит.Представление();
...
//начало блока. Гладилов, 18.07.2014:
Процедура ЗаполнитьНастройкиКолонокПланаВидовРасчета()
	...
	Если МетаданныеИсточника.ДлинаКода > 0 Тогда
		...
		Если МетаданныеИсточника.ТипКода = Метаданные.СвойстваОбъектов.ТипКодаПланаВидовРасчета.Строка Тогда
			ЗагружаемыйРеквизит.ДоступноеОписаниеТипов = Новый ОписаниеТипов("Строка", , Новый КвалификаторыСтроки(МетаданныеИсточника.ДлинаКода));
		Иначе
			ЗагружаемыйРеквизит.ДоступноеОписаниеТипов = Новый ОписаниеТипов("Число", , , Новый КвалификаторыЧисла(МетаданныеИсточника.ДлинаКода));
		КонецЕсли;
...
КонецПроцедуры // ()
//добавлено. Гладилов, 22.07.2014:
Процедура ЗаполнитьНастройкиКолонокРегистраРасчета()
	...
	//Если Не МетаданныеИсточника.ПериодичностьРегистраСведений = Метаданные.СвойстваОбъектов.ПериодичностьРегистраСведений.Непериодический Тогда
	//РР всегда периодические
		
		ЗагружаемыйРеквизит = ТаблицаЗагружаемыхРеквизитов.Добавить();
		ЗагружаемыйРеквизит.ИмяРеквизита           = "ПериодРегистрации";
		ЗагружаемыйРеквизит.ПредставлениеРеквизита = "Период регистрации";
		ЗагружаемыйРеквизит.МожетБытьПолемПоиска = Истина;
		ЗагружаемыйРеквизит.ПолеПоиска           = Истина;
...
//конец блока. Гладилов, 18.07.2014

//19.07.2018 добавляю документы
Процедура ЗаполнитьНастройкиКолонокДокумента()
	...
	Для каждого Реквизит Из МетаданныеИсточника.СтандартныеРеквизиты   Цикл
		Если Реквизит.Имя = "Номер" Тогда
			Продолжить;//будет ниже добавлен
		КонецЕсли;
		ЗагружаемыйРеквизит                        = ТаблицаЗагружаемыхРеквизитов.Добавить();
		ЗагружаемыйРеквизит.ИмяРеквизита           = Реквизит.Имя;
		ЗагружаемыйРеквизит.ПредставлениеРеквизита = Реквизит.Представление();
		ЗагружаемыйРеквизит.ДоступноеОписаниеТипов = МетаданныеИсточника.СтандартныеРеквизиты[ЗагружаемыйРеквизит.ИмяРеквизита].Тип;
		ЗагружаемыйРеквизит.МожетБытьПолемПоиска   = ?(Реквизит.Имя = "Дата",Ложь,Истина);//Гладилов, 19.07.2018. Добавлено;
													//Дату не добавляем т.к. в запросе будет ГДЕ Док.Дата = &Дата, а часто надо нечеткое сравнение (между)
	КонецЦикла;
	//кб
...
#КонецЕсли
//список РежимЗагрузки - см. на форме список значений
